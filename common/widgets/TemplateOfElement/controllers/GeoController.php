...
class GeoController extends Controller
{
     ...
     * TypeAHead для формы
     */
    public function actionGetSearchCountry($query, $lang)
    {
        $manyGeoCountryForm = GeoCountryForm::find()
            ->where(['like', 'name_ru', $query])
            ->orWhere(['like', 'short_name', $query])
     ...
     * TypeAHead для формы
     */
    public function actionGetSearchRegion($query, $lang)
    {
        $session = Yii::$app->session;
        ...
        if (!$id_geo_country) {
            ...
            if (isset($cookiesRequest['id_geo_country_search'])) {
                $id_geo_country = $cookiesRequest['id_geo_country_search']->value;
            }
        }
     ...
     * TypeAHead для формы
     */
    public function actionGetSearchCity($query, $lang)
    {
        $session = Yii::$app->session;
        $cookiesRequest = Yii::$app->request->cookies;
        $id_geo_country = $session->get('id_geo_country_search');
        ...
        if (!$id_geo_region) {
            if (isset($cookiesRequest['id_geo_region_search'])) {
                $id_geo_region = $cookiesRequest['id_geo_region_search']->value;
            }
        }
     ...
     * Установка кук
     */
    public function actionSetSearchCookie($name, $value = null)
    {
        $session = Yii::$app->session;
        $cookiesResponse = Yii::$app->response->cookies;
        $cookiesRequest = Yii::$app->request->cookies;
        ...
        } elseif ($name == 'id_geo_region_search') {
            // если выбран новаый регион, удаляем из кук город
            $session->remove('id_geo_city_search');
            if (isset($cookiesRequest['id_geo_city_search'])) {
                $cookiesResponse->remove('id_geo_city_search');
